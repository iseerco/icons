import React from 'react';
import type { IconProps } from '../../types';

export const GlassWhiskey: React.FC<IconProps> = ({ 
  size = 24, 
  color = 'currentColor',
  className = '',
  ...props 
}) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" width={size} height={size} {...props}><path d="m23.993,4.298c.084-.842-.193-1.685-.762-2.312-.567-.627-1.378-.986-2.224-.986H3.007c-.842.001-1.65.358-2.218.981C.223,2.604-.059,3.441.02,4.28l1.324,14.186c.241,2.586,2.381,4.536,4.979,4.536h11.275c2.58-.001,4.72-1.937,4.976-4.504l1.42-14.199ZM2.268,3.328c.192-.211.454-.327.739-.327h18.001c.286-.001.549.115.741.327.192.212.282.486.254.771l-.99,9.9H2.936s-.925-9.906-.925-9.906c-.027-.284.064-.556.256-.766Zm15.33,17.672H6.322c-1.559.001-2.843-1.168-2.986-2.72l-.213-2.279h17.69s-.23,2.298-.23,2.298c-.153,1.54-1.438,2.701-2.985,2.701Z"/></svg>
);

GlassWhiskey.displayName = 'GlassWhiskey';
