import React from 'react';
import type { IconProps } from '../../types';

export const Tubes: React.FC<IconProps> = ({ 
  size = 24, 
  color = 'currentColor',
  className = '',
  ...props 
}) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" width={size} height={size} {...props}><path d="m10,0H2C1.448,0,1,.447,1,1s.448,1,1,1v18c0,2.206,1.794,4,4,4s4-1.794,4-4V2c.552,0,1-.447,1-1s-.448-1-1-1Zm-4,22c-1.103,0-2-.897-2-2v-9h4v9c0,1.103-.897,2-2,2Zm2-13H4V2h4v7ZM22,0h-8c-.552,0-1,.447-1,1s.448,1,1,1v18c0,2.206,1.794,4,4,4s4-1.794,4-4V2c.552,0,1-.447,1-1s-.448-1-1-1Zm-4,22c-1.103,0-2-.897-2-2v-9h4v9c0,1.103-.897,2-2,2Zm2-13h-4V2h4v7Z"/></svg>
);

Tubes.displayName = 'Tubes';
