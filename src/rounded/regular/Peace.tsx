import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const Peace = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="m12,0C5.383,0,0,5.383,0,12s5.383,12,12,12,12-5.383,12-12S18.617,0,12,0Zm10,12c0,1.864-.513,3.61-1.405,5.105l-7.595-7.595V2.05c5.046.503,9,4.773,9,9.95ZM11,2.05v7.421l-7.61,7.61c-.883-1.49-1.39-3.227-1.39-5.081C2,6.823,5.954,2.553,11,2.05Zm-6.409,16.659l6.409-6.409v9.651c-2.532-.253-4.789-1.453-6.409-3.241Zm8.409,3.241v-9.612l6.391,6.391c-1.619,1.777-3.868,2.969-6.391,3.221Z"/></svg>

);
