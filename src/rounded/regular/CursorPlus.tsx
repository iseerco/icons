import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const CursorPlus = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  <svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="m16 8a8 8 0 1 0 8 8 8.009 8.009 0 0 0 -8-8zm0 14a6 6 0 1 1 6-6 6.006 6.006 0 0 1 -6 6zm4-6a1 1 0 0 1 -1 1h-2v2a1 1 0 0 1 -2 0v-2h-2a1 1 0 0 1 0-2h2v-2a1 1 0 0 1 2 0v2h2a1 1 0 0 1 1 1zm-8-11.019a1 1 0 0 0 -.586-.891l-8.675-3.944a.893.893 0 0 0 -.09-.037 1.99 1.99 0 0 0 -2.061.479 2.011 2.011 0 0 0 -.442 2.151l3.944 8.675a1 1 0 0 0 .891.586h.019a1 1 0 0 0 .895-.553l1.85-3.7 3.7-1.85a1 1 0 0 0 .555-.916zm-5.447 1.124a1 1 0 0 0 -.448.448l-1.064 2.128-3.041-6.676 6.679 3.038z"/></svg>
);
