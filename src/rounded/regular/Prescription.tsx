import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const Prescription = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="m17.869,18l2.964-4.445c.306-.46.182-1.08-.277-1.387-.461-.307-1.081-.182-1.387.277l-2.501,3.752-2.963-4.444c2.481-.736,4.296-3.036,4.296-5.753,0-3.309-2.691-6-6-6h-4.004C5.239,0,2.996,2.243,2.996,5v15c0,.553.448,1,1,1s1-.447,1-1v-8h6.469l4,6-2.963,4.445c-.306.46-.182,1.08.277,1.387.17.113.363.168.554.168.323,0,.64-.156.833-.445l2.501-3.752,2.501,3.752c.193.289.51.445.833.445.19,0,.383-.055.554-.168.459-.307.583-.927.277-1.387l-2.964-4.445ZM4.996,5c0-1.654,1.346-3,3-3h4.004c2.206,0,4,1.794,4,4s-1.794,4-4,4h-7.004v-5Z"/></svg>

);
