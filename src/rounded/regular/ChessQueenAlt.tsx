import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const ChessQueenAlt = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M19,18h-.316A44.01,44.01,0,0,1,16.069,8H17a1,1,0,0,0,0-2h-.472s1.281-2.214,1.294-2.245a1.942,1.942,0,0,0-2.864-2.4l-.677.364S13.461.634,13.432.6a2.045,2.045,0,0,0-2.864,0c-.029.03-.849,1.11-.849,1.11L9.042,1.35a1.942,1.942,0,0,0-2.864,2.4C6.191,3.786,7.472,6,7.472,6H7A1,1,0,0,0,7,8h.931A44.084,44.084,0,0,1,5.316,18H5a3,3,0,0,0,0,6H19a3,3,0,0,0,0-6ZM9.526,3.881A1,1,0,0,0,10.8,3.6L12,2.013,13.2,3.6a1,1,0,0,0,1.272.277l1.409-.758L14.215,6H9.785L8.11,3.119ZM9.939,8h4.122a43.091,43.091,0,0,0,2.487,10h-9.1A43.091,43.091,0,0,0,9.939,8ZM19,22H5a1,1,0,0,1,0-2H19a1,1,0,0,1,0,2Z"/></svg>

);
