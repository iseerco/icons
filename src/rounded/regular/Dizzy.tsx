import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const Dizzy = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M12,0A12.013,12.013,0,0,0,0,12c.6,15.9,23.4,15.893,24,0A12.013,12.013,0,0,0,12,0Zm0,22A10.011,10.011,0,0,1,2,12C2.5-1.248,21.5-1.245,22,12A10.011,10.011,0,0,1,12,22Zm3-5a3,3,0,0,1-6,0A3,3,0,0,1,15,17Zm3.707-9.293L17.414,9l1.293,1.293a1,1,0,0,1-1.414,1.414L16,10.414l-1.293,1.293a1,1,0,0,1-1.414-1.414L14.586,9,13.293,7.707a1,1,0,0,1,1.414-1.414L16,7.586l1.293-1.293A1,1,0,0,1,18.707,7.707ZM8,10.414,6.707,11.707a1,1,0,0,1-1.414-1.414L6.586,9,5.293,7.707A1,1,0,0,1,6.707,6.293L8,7.586,9.293,6.293a1,1,0,0,1,1.414,1.414L9.414,9l1.293,1.293a1,1,0,0,1-1.414,1.414Z"/></svg>

);
