import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const HouseChimneyWindow = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M14,10h-4c-1.103,0-2,.897-2,2v4c0,1.103,.897,2,2,2h4c1.103,0,2-.897,2-2v-4c0-1.103-.897-2-2-2Zm-4,6v-4h4v4h-4ZM22,5.724V2c0-.553-.448-1-1-1s-1,.447-1,1v2.367L14.797,.855c-1.699-1.146-3.895-1.146-5.594,0L2.204,5.579c-1.38,.931-2.204,2.479-2.204,4.145v9.276c0,2.757,2.243,5,5,5h14c2.757,0,5-2.243,5-5V9.724c0-1.581-.744-3.058-2-3.999Zm0,13.276c0,1.654-1.346,3-3,3H5c-1.654,0-3-1.346-3-3V9.724c0-.999,.494-1.928,1.322-2.486L10.322,2.514c.51-.345,1.094-.517,1.678-.517s1.168,.172,1.678,.517l7,4.724c.828,.559,1.322,1.487,1.322,2.486v9.276Z"/></svg>

);
