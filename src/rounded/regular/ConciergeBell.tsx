import React from 'react';
import type { IconProps } from '../../types';

export const ConciergeBell: React.FC<IconProps> = ({ 
  size = 24, 
  color = 'currentColor',
  className = '',
  ...props 
}) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" width={size} height={size} {...props}><path d="M23,21H13v-2h7.343c1.465,0,2.657-1.192,2.657-2.657v-.343c0-3.029-1.268-5.955-3.479-8.027-1.817-1.703-4.09-2.715-6.521-2.93V3h1c.552,0,1-.448,1-1s-.448-1-1-1h-4c-.552,0-1,.448-1,1s.448,1,1,1h1v2.044C5.371,5.553,1,10.462,1,16.343c0,1.465,1.192,2.657,2.657,2.657h7.343v2H1c-.552,0-1,.448-1,1s.448,1,1,1H23c.552,0,1-.448,1-1s-.448-1-1-1ZM3,16.342c0-4.923,3.689-9.018,8.399-9.323,.197-.013,.393-.019,.588-.02h.028c2.294,.002,4.455,.854,6.138,2.432,1.836,1.721,2.847,4.053,2.847,6.568v.343c0,.362-.295,.657-.657,.657H3.657c-.362,0-.657-.295-.657-.658Z"/></svg>
);

ConciergeBell.displayName = 'ConciergeBell';
