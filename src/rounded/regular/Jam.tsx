import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const Jam = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M23.818,7.1A4.782,4.782,0,0,0,20,4.078V3a3,3,0,0,0-3-3H7A3,3,0,0,0,4,3V4.078c-3.776.457-5.682,5.347-2,6.79V18a6.006,6.006,0,0,0,6,6h8a6.006,6.006,0,0,0,6-6V10.87A2.91,2.91,0,0,0,23.818,7.1ZM20,18a4,4,0,0,1-4,4H8a4,4,0,0,1-4-4V10.861c.895-.068,1.658-1.313,2.556-.7a4.058,4.058,0,0,0,4.889,0,.947.947,0,0,1,1.111,0,4.058,4.058,0,0,0,4.889,0c.9-.619,1.658.627,2.555.7Zm1.831-9.346c-.37.618-1.556.281-2.052-.07a2.945,2.945,0,0,0-3.557,0,2.03,2.03,0,0,1-2.443,0,2.945,2.945,0,0,0-3.557,0,2.03,2.03,0,0,1-2.443,0,2.945,2.945,0,0,0-3.557,0c-.84.616-2.535.654-2.165-.791A2.86,2.86,0,0,1,5,6H15a1,1,0,0,0,0-2H6V3A1,1,0,0,1,7,2H17a1,1,0,0,1,1,1V5a1,1,0,0,0,1,1C21.217,6.008,22.413,7.651,21.831,8.654Z"/></svg>

);
