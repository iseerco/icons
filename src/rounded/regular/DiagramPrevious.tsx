import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const DiagramPrevious = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M20,14h-7v-4h1.45c.69,0,1.04-.83,.55-1.32l-2.45-2.45c-.3-.3-.79-.3-1.1,0l-2.45,2.45c-.49,.49-.14,1.32,.55,1.32h1.45v4H4c-2.21,0-4,1.79-4,4v2c0,2.21,1.79,4,4,4H20c2.21,0,4-1.79,4-4v-2c0-2.21-1.79-4-4-4Zm2,6c0,1.1-.9,2-2,2H4c-1.1,0-2-.9-2-2v-2c0-1.1,.9-2,2-2H20c1.1,0,2,.9,2,2v2ZM20,0H4C1.79,0,0,1.79,0,4v2c0,2.21,1.79,4,4,4h1c.27,0,.52-.11,.71-.29L13.41,2h3.17l-1.93,1.93c-.39,.39-.39,1.02,0,1.41,.2,.2,.45,.29,.71,.29s.51-.1,.71-.29l3.35-3.35h.59c.7,0,1.32,.36,1.68,.91l-4.38,4.38c-.19,.19-.29,.44-.29,.71v1c0,.55,.45,1,1,1h2c2.21,0,4-1.79,4-4v-2c0-2.21-1.79-4-4-4ZM2,4c0-1.1,.9-2,2-2h.59l-2.59,2.59v-.59Zm2.59,4h-.59c-.7,0-1.32-.36-1.68-.91L7.41,2h3.17l-6,6Zm17.41-2c0,1.1-.9,2-2,2h-.59l2.59-2.59v.59Z"/></svg>

);
