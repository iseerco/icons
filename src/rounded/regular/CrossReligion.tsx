import React from 'react';
import type { IconProps } from '../../types';

export const CrossReligion: React.FC<IconProps> = ({ 
  size = 24, 
  color = 'currentColor',
  className = '',
  ...props 
}) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" width={size} height={size} {...props}><path d="M12,24c-2.206,0-4-1.794-4-4v-6h-2c-2.206,0-4-1.794-4-4s1.794-4,4-4h2v-2c0-2.206,1.794-4,4-4s4,1.794,4,4v2h2c2.206,0,4,1.794,4,4s-1.794,4-4,4h-2v6c0,2.206-1.794,4-4,4ZM6,8c-1.103,0-2,.897-2,2s.897,2,2,2h3c.552,0,1,.447,1,1v7c0,1.103,.897,2,2,2s2-.897,2-2v-7c0-.553,.448-1,1-1h3c1.103,0,2-.897,2-2s-.897-2-2-2h-3c-.552,0-1-.447-1-1v-3c0-1.103-.897-2-2-2s-2,.897-2,2v3c0,.553-.448,1-1,1h-3Z"/></svg>
);

CrossReligion.displayName = 'CrossReligion';
