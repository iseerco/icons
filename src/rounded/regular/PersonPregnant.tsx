import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const PersonPregnant = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M8.5,2.5c0-1.381,1.119-2.5,2.5-2.5s2.5,1.119,2.5,2.5-1.119,2.5-2.5,2.5-2.5-1.119-2.5-2.5Zm9.5,12c0,1.713-.946,4.512-1.055,4.826-.139,.403-.519,.674-.945,.674h-2v3c0,.552-.447,1-1,1s-1-.448-1-1v-3h-2v3c0,.552-.447,1-1,1s-1-.448-1-1v-3.188c-.474-.169-.909-.441-1.249-.826-.569-.645-.834-1.504-.728-2.357l.893-7.14c.248-1.989,1.947-3.488,3.951-3.488s3.703,1.5,3.951,3.488l.006,.048c1.924,.896,3.176,2.817,3.176,4.964Zm-2,0c0-1.5-.968-2.828-2.408-3.305-.365-.121-.629-.441-.678-.824l-.08-.634c-.124-.991-.97-1.738-1.967-1.738s-1.843,.747-1.967,1.736l-.192,1.534,2.846,1.897c.46,.306,.584,.927,.277,1.387-.192,.289-.51,.445-.833,.445-.19,0-.383-.054-.554-.168l-2.014-1.343-.423,3.387c-.036,.289,.05,.568,.242,.786,.192,.218,.459,.338,.75,.338h6.271c.309-1.006,.729-2.583,.729-3.5Z"/></svg>

);
