import React from 'react';
import type { IconProps } from '../../types';

export const ArrowUpSquareTriangle: React.FC<IconProps> = ({ 
  size = 24, 
  color = 'currentColor',
  className = '',
  ...props 
}) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" width={size} height={size} {...props}><path d="M14,7v-3c0-1.654,1.346-3,3-3h3c1.654,0,3,1.346,3,3v3c0,1.654-1.346,3-3,3h-3c-1.654,0-3-1.346-3-3Zm9.196,12.583l-2.689-4.387c-.399-.737-1.169-1.195-2.007-1.195s-1.608,.458-1.981,1.149l-2.741,4.479c-.387,.714-.37,1.558,.046,2.256s1.149,1.115,1.962,1.115h5.43c.812,0,1.546-.417,1.962-1.115s.433-1.542,.02-2.302ZM11.707,6.293c.391-.391,.391-1.023,0-1.414L7.414,.586C7.024,.196,6.512,0,6,0s-1.024,.195-1.414,.585L.293,4.879c-.391,.391-.391,1.023,0,1.414s1.023,.391,1.414,0l3.293-3.293V23c0,.553,.447,1,1,1s1-.447,1-1V3l3.293,3.293c.391,.391,1.023,.391,1.414,0h0Z"/></svg>
);

ArrowUpSquareTriangle.displayName = 'ArrowUpSquareTriangle';
