import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const UpRight = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M7.87,6.11c-1.1-1.11-1.2-2.82-.11-4.14,.54-.65,1.39-.97,2.23-.97h0s9.01,0,9.01,0c2.21,0,3.99,1.79,3.99,4h0s0,8.88,0,8.88c0,1.17-.61,2.3-1.66,2.81-1.18,.58-2.52,.36-3.45-.56l-1.47-1.47-7.5,7.44c-1.17,1.17-3.07,1.17-4.24,0l-2.83-2.83c-1.17-1.17-1.17-3.07,0-4.24l7.5-7.45-1.48-1.47Z"/></svg>

);
