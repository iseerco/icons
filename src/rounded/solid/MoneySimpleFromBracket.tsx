import React from 'react';
import type { IconProps } from '../../types';

export const MoneySimpleFromBracket: React.FC<IconProps> = ({ 
  size = 24, 
  color = 'currentColor',
  className = '',
  ...props 
}) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" width={size} height={size} {...props}><path d="M24,4v1c0,1.47-.81,2.75-2,3.44V4c0-1.1-.9-2-2-2H4c-1.1,0-2,.9-2,2v4.44c-1.19-.69-2-1.97-2-3.44v-1C0,1.79,1.79,0,4,0H20c2.21,0,4,1.79,4,4Zm-4,15c0,2.76-2.24,5-5,5h-6c-2.76,0-5-2.24-5-5V5c0-.55,.45-1,1-1h14c.55,0,1,.45,1,1v14Zm-4-7c0-2.21-1.79-4-4-4s-4,1.79-4,4,1.79,4,4,4,4-1.79,4-4Zm-4-2c-1.1,0-2,.9-2,2s.9,2,2,2,2-.9,2-2-.9-2-2-2Z"/></svg>
);

MoneySimpleFromBracket.displayName = 'MoneySimpleFromBracket';
