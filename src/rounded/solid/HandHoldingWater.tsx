import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const HandHoldingWater = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M8.5,7.997c-.935,0-1.814-.364-2.475-1.025-1.364-1.364-1.364-3.586,0-4.95L7.801,.285c.389-.381,1.01-.381,1.398,0l1.768,1.729c1.373,1.372,1.373,3.593,.008,4.958h0c-.661,.661-1.54,1.025-2.475,1.025Zm14.652,.684c-.515-.469-1.186-.712-1.878-.678-.697,.032-1.339,.334-1.794,.835l-3.541,3.737c.032,.21,.065,.42,.065,.638,0,2.083-1.555,3.876-3.617,4.17l-4.252,.596c-.547,.078-1.053-.302-1.131-.848-.078-.547,.302-1.053,.848-1.131l4.162-.583c.936-.134,1.748-.806,1.94-1.732,.296-1.425-.79-2.685-2.164-2.685H4.003C1.794,11,.003,12.791,.003,15v5C.003,22.209,1.794,24,4.003,24h4.262c2.805,0,5.48-1.178,7.374-3.246l7.702-8.409c.948-1.062,.862-2.707-.189-3.665Z"/></svg>

);
