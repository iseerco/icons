import React from 'react';
import type { IconProps } from '../../types';

export const SortSizeDown: React.FC<IconProps> = ({ 
  size = 24, 
  color = 'currentColor',
  className = '',
  ...props 
}) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" width={size} height={size} {...props}><path d="M24,4v4c0,1.65-1.35,3-3,3h-4c-1.65,0-3-1.35-3-3V4c0-1.65,1.35-3,3-3h4c1.65,0,3,1.35,3,3Zm-3,11h-2c-1.65,0-3,1.35-3,3v2c0,1.65,1.35,3,3,3h2c1.65,0,3-1.35,3-3v-2c0-1.65-1.35-3-3-3Zm-10.69,3.28l-3.31,3.15V1c0-.55-.45-1-1-1s-1,.45-1,1V21.43l-3.31-3.15c-.4-.38-1.03-.37-1.41,.03-.38,.4-.37,1.03,.03,1.41l3.57,3.4c.57,.57,1.32,.88,2.12,.88s1.55-.31,2.1-.86l3.59-3.41c.4-.38,.42-1.01,.03-1.41-.38-.4-1.01-.42-1.41-.03Z"/></svg>
);

SortSizeDown.displayName = 'SortSizeDown';
