import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const Stamp = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M21,24H3c-.552,0-1-.448-1-1s.448-1,1-1H21c.553,0,1,.448,1,1s-.447,1-1,1Zm3-6c0-2.757-2.243-5-5-5h-3c-.404,0-.653-.223-.766-.356-.111-.133-.289-.416-.22-.813,.58-3.334,.985-6.555,.985-7.831,0-2.206-1.794-4-4-4s-4,1.794-4,4c0,1.278,.405,4.498,.985,7.831,.069,.397-.107,.68-.219,.812-.112,.133-.36,.357-.766,.357h-3c-2.757,0-5,2.243-5,5,0,.97,.701,2,2,2H22c1.299,0,2-1.03,2-2Z"/></svg>

);
