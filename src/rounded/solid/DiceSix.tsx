import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const DiceSix = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M19,0H5A5.006,5.006,0,0,0,0,5V19a5.006,5.006,0,0,0,5,5H19a5.006,5.006,0,0,0,5-5V5A5.006,5.006,0,0,0,19,0ZM7,18a1,1,0,1,1,1-1A1,1,0,0,1,7,18Zm0-5a1,1,0,1,1,1-1A1,1,0,0,1,7,13ZM7,8A1,1,0,1,1,8,7,1,1,0,0,1,7,8ZM17,18a1,1,0,1,1,1-1A1,1,0,0,1,17,18Zm0-5a1,1,0,1,1,1-1A1,1,0,0,1,17,13Zm0-5a1,1,0,1,1,1-1A1,1,0,0,1,17,8Z"/></svg>

);
