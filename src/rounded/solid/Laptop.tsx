import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const Laptop = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M15.848,16a2,2,0,0,0-1.527.708L14.074,17H9.925l-.246-.292A2,2,0,0,0,8.151,16H2.5A2.5,2.5,0,0,0,0,18.5H0A2.5,2.5,0,0,0,2.5,21h19A2.5,2.5,0,0,0,24,18.5h0A2.5,2.5,0,0,0,21.5,16Z"/><path d="M3,14H8.151A3.993,3.993,0,0,1,10.8,15h2.4a4,4,0,0,1,2.647-1H21a4.909,4.909,0,0,1,1,.11V8a5.006,5.006,0,0,0-5-5H7A5.006,5.006,0,0,0,2,8v6.11A4.909,4.909,0,0,1,3,14Z"/></svg>

);
