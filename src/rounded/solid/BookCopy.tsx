import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const BookCopy = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="m10.643,14h13.357v1c0,2.209-1.791,4-4,4h-9.5c-1.466,0-2.637-1.261-2.487-2.758.13-1.301,1.322-2.242,2.63-2.242Zm0-2h13.357V4c0-2.209-1.791-4-4-4h-8c-2.209,0-4,1.791-4,4v8.811c.757-.511,1.671-.811,2.643-.811Zm-4.613,5c-.016-.141-.025-.284-.028-.427h-.002s0-11.573,0-11.573h-2C1.791,5,0,6.791,0,9v8.811c.757-.511,1.671-.811,2.643-.811h3.387Zm4.47,4c-1.269,0-2.485-.54-3.337-1.481-.148-.164-.283-.337-.404-.519H2.643c-1.308,0-2.499.941-2.63,2.242-.15,1.496,1.021,2.758,2.487,2.758h9.5c1.864,0,3.43-1.275,3.874-3h-5.374Z"/></svg>

);
