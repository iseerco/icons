import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const UserNurse = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="m15,0h-6c-1.654,0-3,1.346-3,3v4c0,.553.447,1,1,1h10c.553,0,1-.447,1-1V3c0-1.654-1.346-3-3-3Zm-1,5h-1v1c0,.553-.447,1-1,1s-1-.447-1-1v-1h-1c-.553,0-1-.447-1-1s.447-1,1-1h1v-1c0-.553.447-1,1-1s1,.447,1,1v1h1c.553,0,1,.447,1,1s-.447,1-1,1ZM4.007,24c-.597,0-1.07-.519-1-1.111.241-2.037,1.174-4.221,2.461-5.424.728-.68,1.885-.599,2.546.146l3.986,5.014,3.996-5.009c.662-.744,1.819-.823,2.546-.142,1.282,1.202,2.211,3.382,2.451,5.414.07.593-.403,1.111-1,1.111H4.007Zm1.993-14h12c0,3.309-2.691,6-6,6s-6-2.691-6-6Z"/></svg>

);
