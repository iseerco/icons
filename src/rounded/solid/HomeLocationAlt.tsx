import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const HomeLocationAlt = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M16.243,10.764a6,6,0,0,0-8.486,8.485l3.507,3.43a1.053,1.053,0,0,0,1.472,0l3.507-3.43A6,6,0,0,0,16.243,10.764ZM12,16.994a2,2,0,1,1,2-2A2,2,0,0,1,12,16.994Z"/><path d="M21.8,5.579,14.8.855A4.981,4.981,0,0,0,9.2.855l-7,4.724A4.993,4.993,0,0,0,0,9.724V19a5.006,5.006,0,0,0,5,5H9.754L6.359,20.679A8,8,0,1,1,17.657,9.35h0a8,8,0,0,1,0,11.313L14.245,24H19a5.006,5.006,0,0,0,5-5V9.724A5,5,0,0,0,21.8,5.579Z"/></svg>

);
