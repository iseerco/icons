import React from 'react';
import type { IconProps } from '../../types';

export const ChessKing: React.FC<IconProps> = ({ 
  size = 24, 
  color = 'currentColor',
  className = '',
  ...props 
}) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" width={size} height={size} {...props}><path d="M23.328,10.222a5.038,5.038,0,0,0-3.27-2.389A40.266,40.266,0,0,0,13,7.016V5h2a1,1,0,0,0,0-2H13V1a1,1,0,0,0-2,0V3H9A1,1,0,0,0,9,5h2V7.016a40.266,40.266,0,0,0-7.058.817A5,5,0,0,0,.249,14.27L3.089,22H3a1,1,0,0,0,0,2H21a1,1,0,0,0,0-2h-.089l2.856-7.774A5.011,5.011,0,0,0,23.328,10.222Z"/></svg>
);

ChessKing.displayName = 'ChessKing';
