import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const S = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M16.151,24H7.917c-3.263,0-5.917-2.654-5.917-5.917v-1.083c0-.553,.447-1,1-1s1,.447,1,1v1.083c0,2.16,1.757,3.917,3.917,3.917h8.234c2.122,0,3.849-1.727,3.849-3.849,0-1.732-1.168-3.26-2.839-3.714l-10.846-2.945c-2.541-.689-4.315-3.011-4.315-5.644C2,2.624,4.624,0,7.849,0h8.234c3.263,0,5.917,2.654,5.917,5.917v1.083c0,.553-.447,1-1,1s-1-.447-1-1v-1.083c0-2.16-1.757-3.917-3.917-3.917H7.849c-2.122,0-3.849,1.727-3.849,3.849,0,1.732,1.168,3.26,2.839,3.714l10.846,2.945c2.541,.689,4.315,3.011,4.315,5.644,0,3.225-2.624,5.849-5.849,5.849Z"/></svg>

);
