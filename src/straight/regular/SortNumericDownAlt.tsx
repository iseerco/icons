import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const SortNumericDownAlt = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M18.5,0c-1.93,0-3.5,1.57-3.5,3.5s1.57,3.5,3.5,3.5c.364,0,.709-.072,1.039-.175-.666,1.286-1.994,2.175-3.539,2.175v2c3.309,0,6-2.691,6-6v-1.5c0-1.93-1.57-3.5-3.5-3.5Zm-1.5,3.5c0-.827,.673-1.5,1.5-1.5s1.5,.673,1.5,1.5-.673,1.5-1.5,1.5-1.5-.673-1.5-1.5Zm4,9.5v11h-2v-7.621l-1.284,1.318-1.433-1.395,3.215-3.303h1.501Zm-10.5,4.5l1.414,1.414-4.5,4.5c-.39,.39-.902,.585-1.414,.585s-1.024-.195-1.414-.585L.086,18.914l1.414-1.414,3.5,3.5V0h2V21l3.5-3.5Z"/></svg>

);
