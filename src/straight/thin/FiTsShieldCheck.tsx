import React from 'react';
import type { IconProps } from '../../types';

export const FiTsShieldCheck: React.FC<IconProps> = ({ 
  size = 24, 
  color = 'currentColor',
  className = '',
  ...props 
}) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" {...props}>
  <path d="M11.98,23.986l-.626-.313c-2.194-1.097-9.354-5.189-9.354-11.8V5.135c0-1.08,.688-2.034,1.713-2.373L12,.014l8.287,2.748c1.024,.34,1.713,1.294,1.713,2.373v6.738c0,7.499-7.172,10.968-9.37,11.852l-.65,.261Zm.02-22.918L4.028,3.711c-.615,.204-1.028,.776-1.028,1.424v6.738c0,6.038,6.736,9.874,8.801,10.906l.224,.112,.232-.093c2.051-.825,8.743-4.052,8.743-10.924V5.134c0-.647-.413-1.22-1.027-1.424L12,1.067Z"/>
  <path d="M10.501,15c-.384,0-.769-.146-1.062-.439l-3.288-3.202,.697-.717,3.293,3.207c.201,.2,.518,.199,.712,.005l6.295-6.209,.703,.712-6.293,6.207c-.291,.291-.674,.437-1.057,.437Z"/>
</svg>
);

FiTsShieldCheck.displayName = 'FiTsShieldCheck';
