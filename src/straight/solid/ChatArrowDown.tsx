import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const ChatArrowDown = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><title>02-Diagram</title><path d="M21.207,13.781,15.5,8.074l-2,2-5.8-5.8L6.281,5.683,13.5,12.9l2-2L19.793,15.2,17,17.988h5.5a1.5,1.5,0,0,0,1.5-1.5v-5.5Z"/><path d="M2,20.976V0H0V20.976a3,3,0,0,0,3,3H24v-2H3A1,1,0,0,1,2,20.976Z"/></svg>

);
