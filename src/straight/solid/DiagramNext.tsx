import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const DiagramNext = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M21,0H3C1.35,0,0,1.35,0,3v7H11v4h-3l3.18,3.65c.44,.49,1.22,.49,1.66,0l3.16-3.65h-3v-4h11V3c0-1.65-1.35-3-3-3Zm.94,2.65l-5.35,5.35h-3.17l6-6h1.59c.43,0,.79,.27,.94,.65Zm-11.35,5.35h-3.17L13.41,2h3.17l-6,6ZM3,2h1.59l-2.59,2.59v-1.59c0-.55,.45-1,1-1Zm-1,5.41L7.41,2h3.17l-6,6H2v-.59Zm17.41,.59l2.59-2.59v2.59h-2.59Zm-.77,6h5.36v7c0,1.65-1.35,3-3,3H3c-1.65,0-3-1.35-3-3v-7H5.35l4.35,4.99c.59,.66,1.43,1.03,2.32,1.03h0c.88,0,1.73-.38,2.34-1.06l4.29-4.96Z"/></svg>

);
