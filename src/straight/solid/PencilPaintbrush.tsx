import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const PencilPaintbrush = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="m18.313,9.396L3.71,24H0v-3.71L14.604,5.687l3.71,3.71ZM23.232.768c-1.024-1.024-2.685-1.024-3.71,0l-3.504,3.504,3.71,3.71,3.504-3.504c1.024-1.024,1.024-2.685,0-3.71ZM6.667,10.795l3.71-3.71L4.478.768C3.454-.256,1.793-.256.768.768-.256,1.793-.256,3.454.768,4.478l5.899,6.317Zm16.977,11.798c-.717-2.836-2.519-7.592-6.54-7.592-2.263,0-4.104,2.019-4.104,4.5,0,2.535,2.511,4.5,4.671,4.5h6.329l-.356-1.408Z"/></svg>

);
