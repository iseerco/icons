import * as React from 'react';

interface Props extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  color?: string;
  className?: string;
}

export const Meteor = ({ size = 24, color = 'currentColor', className, ...props }: Props) => (
  
<svg {...props} width={size} height={size} viewBox="0 0 24 24" fill={color} className={className}><path d="M11,15a2,2,0,0,1-4,0A2,2,0,0,1,11,15Zm9.289-7h2.738l-.61,1.4c-.155.356-3.832,8.744-7.053,11.965A9,9,0,0,1,2.636,8.636C5.857,5.415,14.245,1.738,14.6,1.583L16,.974V3.811L23.745.225ZM13,15c-.138-5.275-7.863-5.274-8,0C5.138,20.275,12.863,20.274,13,15Z"/></svg>

);
