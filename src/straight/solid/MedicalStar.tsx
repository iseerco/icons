import React from 'react';
import type { IconProps } from '../../types';

export const MedicalStar: React.FC<IconProps> = ({ 
  size = 24, 
  color = 'currentColor',
  className = '',
  ...props 
}) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" width={size} height={size} {...props}><polygon points="22.715 6.586 21.67 4.881 12.959 10.215 12.959 0 10.959 0 10.959 10.215 2.248 4.881 1.203 6.586 10.044 12 1.203 17.414 2.248 19.119 10.959 13.785 10.959 24 12.959 24 12.959 13.785 21.67 19.119 22.715 17.414 13.874 12 22.715 6.586"/></svg>
);

MedicalStar.displayName = 'MedicalStar';
